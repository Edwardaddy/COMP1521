...
#   {
#    if (row == col && m[row][col] != 1) return 0;
#    else if (row != col && m[row][col] != 0) return 0;
#   }
#  }
#  return 1;
# }
move $s0, $a0 #`m' in [reg] (from $a0) -> s0
move $s1, $a1 #`N' in [reg] (from $a1) -> s1
row_ini:
li $s2, 0  # row = 0
row_cond:
bge $s2, $s1, row_end # row < n  ||  row >= n
nop #[branch delay]

col_ini:
li $s3, 0  # col = 0
col_cond:
bge $s3, $s1, col_end # col < n  ||  col >= n
nop #[branch delay]
...
lw $t0, ($t0)  # load word from ($t0) to $t0, (store m[row][col] to $t0)
beq  $s2, $s3, if1 # if row = col
j if2
if1: # if (row == col && m[row][col] != 1) return 0;
li $t1, 1  # load 1 to $t1
bne  $t0, $t1, not_identity
j cont
if2: # else if (row != col && m[row][col] != 0) return 0;
