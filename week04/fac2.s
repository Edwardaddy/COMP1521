...
fac:
	...
	li $s1, 1		# prod = 1
while:
	# 4. conditionally branch instruction: compare counter i ($s1) to n ($s0)
	bgt $s0, $a0, end_while	# if i > n, end_while
	mul $s1, $s1, $s0		# num = num*i
	addi $s0, $s0, 1		# adds 1 to $s1 then load results to $s1
	j while

end_while:
	move $v0, $s1		# place n! in $v0
