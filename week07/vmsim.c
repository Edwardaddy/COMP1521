...
int physicalAddress (uint vAddr, char action)
{
	...
	if (PageTable[pagenum].frameNo != -1) {
		...
		// use the frame number and offset to compute a physic address
		...
		for (int i = 0; i < nFrames; i++) {
			if (MemFrames[i] == -1) {
				...
				PageTable[pagenum].lastAccessed = clock; // access time = current clock tick
		...
		// oterwise;
		// we need to replace a currently loaded frame,
		...
		// j is recently used loaded page, srt it's Pagetable entry to indicate "no longer loaded"
		...
		// set itspagetable entry to inf=dicate "no longer loaded"
